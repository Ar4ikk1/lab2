package utils;

public class ErrorMessages {

    public static final String DAO_ERROR = "Error in DAO";
    public static final String SERVICE_ERROR = "Error in Service";
    public static final String CAR_ALREADY_EXISTS = "Car with this license plate already exists";
    public static final String INVALID = "Invalid";
    public static final String BRAND_INVALID = "Brand name must be between 2 and 30 characters";
    public static final String MODEL_INVALID = "Model name must be between 1 and 50 characters";
    public static final String COLOR_INVALID = "Incorrect Color name";
    public static final String LICENSE_PLATE_INVALID = "Incorrect License Plate format (expected AA1111AA)";
}
